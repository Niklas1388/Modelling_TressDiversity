...
# Analyse of abundance
## The commonness, and rarity of species
### plotting rel.abundaces ECDF
```{r}
# abundances/plot
...
# Biodiversity
## hill-diversity or hill-numbers
...
## generate dataset: parameter per plot
```{r}
...
```{r}
# VSURF
loadandinstall("VSURF")
str(vegIndices)
result <- VSURF(vegIndices, basal_Cerrado[,60], ntree = 100, parallel = F, clusterType = "FORK",  na.action = na.omit)

str(basal_Cerrado[,24:25])
result$call
names(result)
result$varselect.interp
result$varselect.interp
result$varselect.pred
summary(result)
plot(result)

loadandinstall("dplyr")
test <- select(basal_Cerrado, specrich, Shannon_hill_q1, Simpson_hill_q2, rarefaction, rao, shannon, entropyndvi, maxndvi, minndvi, meanndvi, sdndvi)
str(test)
str(basal_Cerrado)
str(basal_Cerrado[,1:52])
str(basal_Cerrado[,57:60])
# BORUTA RANDOM FOREST
loadandinstall("Boruta")
# Decide if a variable is important or not using Boruta
boruta_output <- Boruta(Coll_Cerrado, basal_Cerrado[,59], pValue = 0.05, doTrace=2, mcAdj = TRUE, maxRuns = 30)  # perform Boruta search
...
loadandinstall("Boruta")
# Decide if a variable is important or not using Boruta
boruta_output <- Boruta(test[,5:11], test[,3], pValue = 0.01, doTrace=2, mcAdj = TRUE, maxRuns = 15)  # perform Boruta search
# Confirmed 10 attributes: Humidity, Inversion_base_height, Inversion_temperature, Month, Pressure_gradient and 5 more.
# Rejected 3 attributes: Day_of_month, Day_of_week, Wind_speed.
boruta_signif <- names(boruta_output$finalDecision[boruta_output$finalDecision %in% c("Confirmed", "Tentative")])  # collect Confirmed and Tentative variables
print(boruta_signif)
...
# MARS --> based on generalised cross validation (GCV)
loadandinstall("earth")
marsmodel <- earth(basal_Cerrado[,1:52], basal_Cerrado[,57])
ev <- evimp(marsmodel)
plot(ev)

#random forest
loadandinstall("party")
cf1 <- cforest(rarefaction ~ ., data=test, controls = cforest_unbiased(mtry=2, ntree=50))
varimp(cf1)
varimp(cf1, conditional = TRUE)
varimpAUC(cf1)

names(basal_Cerrado[,5:34])

#!!not work # calc.relimp --> relative importance
loadandinstall("relaimpo")
lmMod <- lm(Shannon_hill_q1 ~ ., test)
relImportance <- calc.relimp(lmMod, type = "lmg", rela = TRUE)  # calculate relative importance scaled to 100
sort(relImportance$lmg, decreasing=TRUE)  # relative importance
```


## Variable Selection (best model) Package (leaps) based on adjustedR?
### Cerrado
```{r}
####################################
#                                  #
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model

model.min <- lm(Simpson_hill_q2 ~ 1, data = basal_nomixel[basal_nomixel$Biome == "Cerrado",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
...
model.SR.AIC <- stepAIC(model.min, direction="both",
                     scope=list(upper=model.max, lower=model.min))
# stepAIC possible directions = "both" "forward" "backward"
...
model.SR.AIC
stat$coefficients[,1]
# best models retains intercept, other variables are below 5%-level and not significant
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(rarefaction ~ meanndvi + sdndvi  + minndvi + entropyndvi + rao + meantcb + minndwi, data = basal_nomixel[basal_nomixel$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Shannon_hill_q1 ~ meanndvi + sdndvi  + minndvi + entropyndvi + rao + meantcb + minndwi, data = basal_nomixel[basal_nomixel$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(Shannon_hill_q1 ~ meanndvi + sdndvi  + minndvi + entropyndvi + rao + meantcb + minndwi, data = basal_nomixel[basal_nomixel$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Simpson_hill_q2 ~ meanndvi + sdndvi  + minndvi + entropyndvi + rao + meantcb + minndwi, data = basal_nomixel[basal_nomixel$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(Simpson_hill_q2 ~ 1, data = basal[basal$Biome == "Cerrado",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(Simpson_hill_q2 ~ meanndvi + sdndvi  + minndvi + entropyndvi + rao + meantcb + minndwi, data = basal_nomixel[basal_nomixel$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
##
#result
#potential explantaory variables are specrich/rarefactio ~ maxNDVI
...
#staring-model:
model_SR <- lm(specrich ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(specrich ~ 1, data = basal[basal$Biome == "Denso",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(specrich ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(rarefaction ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(rarefaction ~ 1, data = basal[basal$Biome == "Denso",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(rarefaction ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(Shannon_hill_q1 ~ 1, data = basal[basal$Biome == "Denso",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(Simpson_hill_q2 ~ 1, data = basal[basal$Biome == "Denso",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(specrich ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(specrich ~ 1, data = basal[basal$Biome == "Cerrado",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(specrich ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(rarefaction ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(rarefaction ~ 1, data = basal[basal$Biome == "Cerrado",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(rarefaction ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(Shannon_hill_q1 ~ 1, data = basal[basal$Biome == "Cerrado",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(AGBplot ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(AGBplot ~ 1, data = basal[basal$Biome == "Cerrado",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(AGBplot ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(AGBplot ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(AGBplot ~ 1, data = basal[basal$Biome == "Denso",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(AGBplot ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
# all variables with collinearity (Vif >= 4 have been removed)
# minimal model
model.min <- lm(WD ~ 1, data = basal[basal$Biome == "Denso",])
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(WD ~ meanNDVI + sdNDVI + maxNDVI + minNDVI + meansavi +sdsavi + meanevi + sdevi + meanndmi + sdndmi + meanndwi + sdndwi + meanmndwi + sdmndwi  + sdtcg + meantcg + sdtcb + meantcb + sdtcw + meantcw + meannbr + sdnbr, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(specrich ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(specrich ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(rarefaction ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(rarefaction ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Cerrado",])
model.max <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Cerrado",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(specrich ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(specrich ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(rarefaction ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(rarefaction ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(Shannon_hill_q1 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(Simpson_hill_q2 ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
# maximal model (mit allen erkl?renden Variablen)
#model.max <- lm(sdNDVI ~ specrich + rarefaction +  Shannon_hill_q1 + Simpson_hill_q2 + basalarea + StemDensity + dbh + height + AGBplot + WD + dist, data = basal[basal$Biome == "Denso",])
model.max <- lm(AGBplot ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
# load package
library(MASS)
# slect model with stewpise regression "both" (== forward & backward selection)
model.SR.AIC <- stepAIC(model.min, direction="both",
...
#staring-model:
model_SR <- lm(AGBplot ~ meanNDVI + sdNDVI +  meanevi + sdevi, data = basal[basal$Biome == "Denso",])
#vif(model_SR)
anova(model_SR)
summary(model_SR)
